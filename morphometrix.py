...
#2019/06/25
# -upgrade to pyqt5 (done)

#Wish list (descending priority)
#   -yapf it (Formatting) https://yapf.now.sh/
#   -photo saved is fucked up sometimes (v1.5)
#   -exported image has whitespace (v1.5)
#   -test undo (v1.5)
#   -mouseover xy position (v2.0)

#   -tune bezier curve tension parameter on length with scroll wheel (save for patch)
#   -combine into one window (save for patch)
#   -angle measurement fucked up (save for patch)
...
class Second(QMainWindow):
    ...
    def __init__(self,parent=None):
        ...
        self.lengthbutton = QPushButton("Measure Total Length",self) #Add options upon file_open instead?
    ...
    def file_open(self):
        ...
        self.iw.pixmap_fit = self.iw.pixmap.scaled(self.iw.pixmap.width(),self.iw.pixmap.height(),
        QtCore.Qt.KeepAspectRatio, transformMode=QtCore.Qt.SmoothTransformation)
        self.iw.scene.clear()
        self.iw.scene.addPixmap(self.iw.pixmap_fit) #add image
        self.iw.setScene(self.iw.scene)

        #Adjust window size automatically?
        self.setGeometry( QtCore.QRect(QtCore.QPoint(0,0),QtCore.QSize(1000,1000)) ) #change main window size
        ...
        self.iw.numwidths = int(self.parent().numwidths.text()) #fetch # of length segments upon file open
        self.lengthnames = []
        self.iw.widthnames = ['{0:2.2f}% Width'.format(100*f/self.iw.numwidths) for f in np.arange(1,self.iw.numwidths)]
        self.iw.nm = len(self.iw.widthnames) #number of possible measurements per segment (length+ #widths)
        ...
        self.iw.m = None
        self.iw.A = posData(np.empty(shape = (0,0)),np.empty(shape = (0,0))) #lengths
        self.iw.W = posData(np.empty(shape = (0,0)),np.empty(shape = (0,0))) #widths
    ...
    def measure_custom(self):
        ...
        self.iw._lastpos = None
        self.iw._thispos = None
        self.iw.A = posData(np.empty(shape = (0,0)),np.empty(shape = (0,0))) #preallocate custom length
        self.statusBar.showMessage('Click initial point for length measurement')
    ...
    def undo(self):
        ...
        if self.iw.measuring_widths:
            ...
            self.iw.d[str(self.iw.k)].setPen(QtGui.QPen(QtGui.QColor('black')))#un-highlight next spine
    ...
    def export_meas(self):
        fac = max(self.iw.pixmap.width(),self.iw.pixmap.height())/max(self.iw.pixmap_fit.width(),self.iw.pixmap_fit.height()) #scale pixel -> m by scaled image
        name = QFileDialog.getSaveFileName(self, 'Save File',self.image_name[0].split('.',1)[0])[0]
        self.pixeldim = float(self.parent().pixeldim.text()) #change this to correct fraction (cm/pixel)
        self.altitude = float(self.parent().altitude.text())
        self.focal = float(self.parent().focal.text()) #okay in mm https://www.imaging-resource.com/PRODS/sony-a5100/sony-a5100DAT.HTM

        measurements = self.iw.measurements*(fac* self.pixeldim * self.altitude/self.focal) #convert pixels to meters
        optical = np.array([self.parent().id.text(),self.image_name[0],self.focal, self.altitude, self.pixeldim])
        names_optical = ['Image ID','Image Path','Focal Length','Altitude','Pixel Dimension']
        ...
        with open(name +'.csv','w') as csvfile:
            ...
            for k,f in enumerate(names_lengths): #write lengths and widths
                line = [ [f] + measurements[k,:].ravel().tolist() ] #need toconvert NaNs to empty
                writer.writerows(line)

            writer.writerow([''])
            writer.writerow(['Object Name']+['Angle'])
            for k,f in enumerate(self.anglenames): #write angles
                line = [ [f] + [self.iw.T.t[k]] ] #need toconvert NaNs to empty
                writer.writerows(line)
        ...
        self.setGeometry( QtCore.QRect(QtCore.QPoint(0,0),QtCore.QSize(1000,1000)) ) #change main window size
        self.iw.fitInView(self.iw.scene.sceneRect(), QtCore.Qt.KeepAspectRatio)
        pix = QtGui.QPixmap(1000,1000)#self.iw.view.viewport().size())#self.iw.pixmap_fit.copy
...
class imwin(QGraphicsView): #Subclass QLabel for interaction w/ QPixmap
    ...
    def mouseMoveEvent(self,event):
        ...
        #dragging line
        elif self._thispos and (self.measuring_length or self.measuring_angle): #only on mouse press
            if self.measuring_length:
                self.parent().statusBar.showMessage('Click to place next point... double click to finish')
            if self.measuring_angle:
                self.parent().statusBar.showMessage('Click point to define vector')
    ...
    def mouseDoubleClickEvent(self,event):
        ...
        if self.measuring_length and self.parent().bezier.isChecked(): #only delete lines if bezier fit
        ...
        self.measuring_length = False #end length measuring
        self.measuring_angle = False
        self._thispos = False
        ...
        #https://codeplea.com/introduction-to-splines
            n = max(1000,self.numwidths*50) #number of interpolating points, minimum 1000, more if necessary
            ...
            if self.parent().bezier.isChecked():
                ...
                def bezier_curve(points, nTimes=n):
                    ...
                    slopes = np.gradient(yvals)/np.gradient(xvals) #change with analytic gradient
                ...
                add = np.concatenate( (self.l[-1],np.empty(self.nm)*np.nan), axis = None ) #add length and row of nans for possible widths
                ...
                for i in range(1,n-1):
                    start = self.mapFromScene(self.mapToScene(self.xs[i-1],self.ys[i-1])) #+ self.pos()
                    mid = self.mapFromScene(self.mapToScene(self.xs[i],self.ys[i])) #+ self.pos()
                    end = self.mapFromScene(self.mapToScene(self.xs[i+1],self.ys[i+1]))# + self.pos()
            ...
            if not self.parent().bezier.isChecked():
                ...
                # tck, u = splprep([self.A.x, self.A.y], s=0,k=1)
                # new_points = splev(u, tck)
                # self.xs, self.ys = new_points[0] , new_points[1]
                #slopes = np.gradient(self.xs)/np.gradient(self.ys)

                # pts = np.array(list(map(qpt2pt,self.xs,self.ys)))
                pts = np.array(list(map(qpt2pt,self.A.x,self.A.y)))
                x,y = pts[:,0], pts[:,1]

                self.l = np.cumsum( np.hypot(np.diff(x),np.diff(y)) ) #integrate for length
                add = np.concatenate( (self.l[-1],np.empty(self.nm)*np.nan), axis = None ) #add length and row of nans for possible widths
                add = np.expand_dims(add,axis=0)
                self.measurements = np.append(self.measurements,add,axis=0)

                # for i in range(0,len(new_points)-1):
                #     start = self.mapFromScene(self.mapToScene(self.xs[i],self.ys[i])) #+ self.pos()
                #     end = self.mapFromScene(self.mapToScene(self.xs[i+1],self.ys[i+1]))# + self.pos()
                #     path = QtGui.QPainterPath(start)
                #     path.lineTo(end)
                #     self.scene.addPath(path)
    ...
    def measure_widths(self):
        ...
        self.k = 0
        self.W = posData(np.empty(shape = (0,0)),np.empty(shape = (0,0))) #preallocate custom widths
        self.nspines = 2*(self.numwidths-1)
        self.parent().statusBar.showMessage('Click point along spines to make width measurements perpindicular to the length segment')
        #Draw widths
    ...
    #http://pyqt.sourceforge.net/Docs/PyQt4/qgraphicsscenemouseevent.html
            ...
            if self.scene.testline and (self.measuring_length or self.measuring_angle) and self._thispos:
                start = self._thispos
                ...
                if self._lastpos and self.measuring_angle:
                    ...
                    b = np.array( [self._thispos.x() - start.x(), self._thispos.y() - start.y()] )
            ...
            if self.measuring_widths: #measure widths, snap to spines
                ...
                self.scene.ellipseItem.setBrush(QtGui.QBrush(QtCore.Qt.red, style = QtCore.Qt.SolidPattern))
                self.scene.ellipseItem.setFlag(QGraphicsItem.ItemIgnoresTransformations, False) #size stays small, but doesnt translate if false
                ...
                if self.k==self.nspines:
                    ...
                    self.measuring_widths = False
                    width = np.sqrt( (self.W.x[1::2]-self.W.x[0::2])**2 + (self.W.y[1::2]-self.W.y[0::2])**2 ) #calculate widths
...
class angleData(): #do i actually need separate class from posdata? probably not
    def __init__(self,t):
        self.t = t
...
class Window(QWidget):
    def __init__(self,parent=None): #init methods runs every time (core application stuff)
    ...
    def close_application(self):
        choice = QMessageBox.question(self, 'exit', "Exit program?",QMessageBox.Yes | QMessageBox.Yes)
